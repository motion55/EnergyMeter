...
{
#ifdef _GNSS_
	if (AT_RESP_ERR_DIF_RESP == gsm.SendATCmdWaitResp(F("AT+CGNSPWR=1"), 500, 100, str_ok, 5))
		return 0;
...
{
#ifdef _GNSS_
	if (AT_RESP_ERR_DIF_RESP == gsm.SendATCmdWaitResp(F("AT+CGNSPWR=0"), 500, 100, str_ok, 5))
		return 0;
#else
...
char GPSGSM::getPar(char *str_long, char *str_lat, char *str_alt, char *str_time, char *str_speed)
{
	...
	char *p_char1;
#ifdef _GNSS_
	 gsm.SimpleWriteln(F("AT+CGNSINF=0"));
#else
